...
class Belco_Widget_Model_Api
{
    ...
    /**
     * Makes from an array of Magento customers an array of Belco_Hooks_Model_BelcoCustomer's
     * and sends it to the Belco API.
     *
     * @param $customer
     * @return mixed
     */
    ...
    /**
     * Makes from an array of Magento orders an array of Belco_Hooks_Model_BelcoOrder's
     * and sends it to the Belco API.
     *
     * @param $order
     * @return mixed
    ...
    {
        $belcoCustomers = array();
        foreach ($customers as $customer) {
            $belcoCustomers[] = $this->toBelcoCustomer($customer);
        }
        return $this->import('sync/customer', $belcoCustomers);
    }
    ...
    public function import_orders($orders)
    {
        ...
        foreach ($orders as $order) {
            $belcoOrders[] = $this->toBelcoOrder($order);
        }
        return $this->import('sync/order', $belcoOrders);
    }

    private function import($path, $data)
    {
        $config = Mage::getStoreConfig('settings/general');
        ...
        foreach ($chunks as $chunk) {
            sleep($config['sync_timeout']);
            $message = $this->post($path, $chunk);
        }
